...
import hierarchical_cnf as mdl
from hierarchical_cnf import PosteriorNet
...
# IMPORTANT: CNFs are trained in fp32; keep AMP disabled for ODE/divergence stability
AMP_ENABLED = False
...
# solves an ODE and computes the divergence. 64 is the established stable default.
BATCH_SIZE = 64
EPOCHS     = 150

# New self-attention CNF checkpoints are not compatible with older EA CNF checkpoints.
RESUME = False
...
# CNF solver settings (kept from the established CNF configuration)
CNF_ATOL      = 1e-3
CNF_RTOL      = 1e-3
CNF_METHOD    = "rk4"
CNF_STEP_SIZE = 0.1
...
SAVE_DIR = "cnfs_pred_phase_masked_hierarchical_self_attention"
os.makedirs(SAVE_DIR, exist_ok=True)

SAVE_PATH = os.path.join(SAVE_DIR, "best_posterior_flow_cnfs_pred_phase_masked_hierarchical_self_attention.pt")
LAST_PATH = os.path.join(SAVE_DIR, "last_posterior_flow_cnfs_pred_phase_masked_hierarchical_self_attention.pt")
CSV_PATH  = os.path.join(SAVE_DIR, "loss_log_cnfs_pred_phase_self_attention.csv")
...
def make_checkpoint(
    ...
    ckpt = {
        ...
        "THETA_DIM": THETA_DIM,
        "flow_type": "cnf",
        "model_module": "hierarchical_cnf",
        "attention_type": "self_attention",
        "architecture_version": "cnf_hierarchical_self_attention_exact_v1",
        "cnf_divergence": "exact",
        "phase_gate": "explicit",
        "P": P,
        "L": L,
        "X_mean": X_mean,
        ...
        "cnf_atol": CNF_ATOL,
        "cnf_rtol": CNF_RTOL,
        "cnf_method": CNF_METHOD,
        "cnf_step_size": CNF_STEP_SIZE,
        "cnf_hidden": CNF_HIDDEN,

        "use_true_phase": USE_TRUE_PHASE,
        "use_phase_provider": USE_PHASE_PROVIDER,
        "use_phase": USE_PHASE,
    }
...
model = PosteriorNet(
    ...
    rtol=CNF_RTOL,
    method=CNF_METHOD,
    step_size=CNF_STEP_SIZE,
).to(device)

# Verify CNF/model configuration before training.
if model.phase_target_idx != phase_target_idx:
    raise RuntimeError(
        f"Model phase_target_idx={model.phase_target_idx} does not match "
        f"runner phase_target_idx={phase_target_idx}."
    )

if model.flow.theta_dim != THETA_DIM:
    raise RuntimeError(
        f"CNF theta_dim={model.flow.theta_dim} but runner THETA_DIM={THETA_DIM}."
    )

if model.flow.method != CNF_METHOD or abs(model.flow.step_size - CNF_STEP_SIZE) > 1e-15:
    raise RuntimeError(
        "CNF solver configuration does not match the runner configuration."
    )
...
if not has_self_attention:
    raise RuntimeError(
        ...
        "Check that hierarchical_cnf imports HierarchicalPTAEncoder from pta_encoder."
    )
...
if RESUME:
    ...
    if os.path.exists(resume_path):
        ...
        ckpt_flow = ckpt.get("flow_type")
        if ckpt_flow != "cnf":
            raise RuntimeError(
                f"Checkpoint flow_type={ckpt_flow!r}; expected 'cnf'."
            )

        ckpt_arch = ckpt.get("architecture_version")
        if ckpt_arch != "cnf_hierarchical_self_attention_exact_v1":
            raise RuntimeError(
                f"Checkpoint architecture_version={ckpt_arch!r}; this runner requires "
                "'cnf_hierarchical_self_attention_exact_v1'. Start a fresh run for "
                "older CNF checkpoints."
            )

        if ckpt.get("cnf_divergence") != "exact":
            raise RuntimeError(
                "Checkpoint was not trained with the current exact-divergence CNF. "
                "Do not mix it with this run."
            )

        if ckpt.get("phase_gate") != "explicit":
            raise RuntimeError(
                "Checkpoint does not record the current explicit phase-gate design. "
                "Do not mix it with this run."
            )

        # Reject older external-attention checkpoints explicitly.
        ...
        if any(".Mk" in key or ".Mv" in key for key in ckpt_keys):
            raise RuntimeError(
                ...
                "Use the new self-attention CNF checkpoint directory and retrain."
            )
...
# hierarchical_cnf locally re-enables gradients with respect to the ODE state
# only for exact divergence, so outer no_grad is safe for validation.
@torch.no_grad()
...
def per_target_z2(model, loader):
    model.eval()
    acc = torch.zeros(len(target_names), dtype=torch.float64)
    nobs = 0

    for batch in loader:
        xb, phib, yb = prepare_batch(batch, device)
        # z^2 needs only theta -> z, not the CNF log-Jacobian.
        # Use the state-only reverse ODE to avoid exact-divergence work here.
        z = model.to_base(yb, xb, phib if model.use_phase else None)
        ...
        bs = z.size(0)
        acc += z2.sum(dim=0).detach().double().cpu()
        nobs += bs

    return acc / max(nobs, 1)
